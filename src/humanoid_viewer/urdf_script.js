...
class ProfessionalURDFViewer {
  constructor() {
    ...
    this.infoPopupTimeout = null; // Manages the info pop up timeouts
  ...
  /**
   * Displays a temporary informational popup on the screen.
   * @param {string} message The message to display in the popup.
   */
  showInfoPopup(message) {
    // THIS METHOD BELONGS HERE NOW
    let popup = document.getElementById("infoPopup");
    if (!popup) {
      // Create the popup element if it doesn't exist
      popup = document.createElement("div");
      popup.id = "infoPopup";
      // Basic styling for the popup - you can customize this in your CSS
      popup.style.cssText = `
    position: fixed;
    ...
    z-index: 9999;
    top: 280px; /* Adjust top position as needed, e.g., 20px from the top of the viewport */
    left: 370px; /* Assuming sidebar is 350px, plus 20px margin to its right */
    right: auto; /* Ensure it's not constrained by the right edge */
...
`;
    ...
    // Clear any existing timeout to prevent rapid clicks from hiding it too soon
    if (this.infoPopupTimeout) {
      clearTimeout(this.infoPopupTimeout);
    }

    // Set a new timeout to hide the popup after a few seconds
    this.infoPopupTimeout = setTimeout(() => {
      popup.style.opacity = "0";
      popup.style.visibility = "hidden";
    }, 4000); // Popup stays visible for 4 seconds
  }
  ...
  setupEventListeners() {
    ...
    const checkInputs = () => {
      ...
      if (urdfInput.files.length) {
        ...
        urdfWrapper.querySelector(
          ".file-input-text"
        ).innerHTML = `<strong>Selected:</strong> ${urdfInput.files[0].name}`;
      } else {
    ...
    if (meshInput) {
      ...
      meshInput.addEventListener("change", (e) => {
        ...
        if (e.target.files.length) {
          ...
          meshWrapper.querySelector(
            ".file-input-text"
          ).innerHTML = `<strong>Selected:</strong> ${e.target.files.length} mesh files`;
        } else {
  ...
  processMeshFiles(files) {
    ...
    this.updateStatus(
      `Loaded ${files.length} potential mesh files for lookup.`,
      "success"
    );
  }
  ...
  async loadRobotFromFileInput() {
    ...
    if (!urdfInput) {
      console.warn("urdfInput element not found. Skipping file input loading.");
      return;
    }

    const urdfFile = urdfInput.files[0];
    if (!urdfFile) {
      this.updateStatus("Please select a URDF file.", "error");
      return;
    }
    ...
    this.updateStatus("Loading robot...", "loading");
    if (loadButtonText) {
      loadButtonText.innerHTML =
        '<div class="loading-spinner"></div>Loading...';
    }

    try {
      const urdfContent = await this.readFile(urdfFile);
      await this._loadRobotInternal(urdfContent);
      this.updateStatus("Robot loaded successfully!", "success");
      if (loadButtonText) {
        loadButtonText.textContent = "Load Robot";
      }
    } catch (error) {
      console.error("Error loading robot:", error);
      this.updateStatus("Error loading robot: " + error.message, "error");
  ...
  async loadPoppyHumanoid() {
    ...
    meshFilesMap.set(
      "abdomen_respondable.STL",
      "/assets/robot2/meshes/abdomen_respondable.STL"
    );
    meshFilesMap.set(
      "abdomen_visual.STL",
      "/assets/robot2/meshes/abdomen_visual.STL"
    );
    meshFilesMap.set(
      "abs_motors_respondable.stl",
      "/assets/robot2/meshes/abs_motors_respondable.stl"
    );
    meshFilesMap.set(
      "abs_motors_visual.STL",
      "/assets/robot2/meshes/abs_motors_visual.STL"
    );
    meshFilesMap.set(
      "bust_motors_respondable.stl",
      "/assets/robot2/meshes/bust_motors_respondable.stl"
    );
    meshFilesMap.set(
      "bust_motors_visual.STL",
      "/assets/robot2/meshes/bust_motors_visual.STL"
    );
    meshFilesMap.set(
      "chest_respondable.STL",
      "/assets/robot2/meshes/chest_respondable.STL"
    );
    meshFilesMap.set(
      "chest_visual.STL",
      "/assets/robot2/meshes/chest_visual.STL"
    );
    meshFilesMap.set(
      "head_respondable.stl",
      "/assets/robot2/meshes/head_respondable.stl"
    );
    meshFilesMap.set(
      "head_visual.STL",
      "/assets/robot2/meshes/head_visual.STL"
    );
    meshFilesMap.set(
      "l_foot_respondable.stl",
      "/assets/robot2/meshes/l_foot_respondable.stl"
    );
    meshFilesMap.set(
      "l_foot_visual.STL",
      "/assets/robot2/meshes/l_foot_visual.STL"
    );
    meshFilesMap.set(
      "l_forearm_respondable.stl",
      "/assets/robot2/meshes/l_forearm_respondable.stl"
    );
    meshFilesMap.set(
      "l_forearm_visual.STL",
      "/assets/robot2/meshes/l_forearm_visual.STL"
    );
    meshFilesMap.set(
      "l_hip_motor_respondable.stl",
      "/assets/robot2/meshes/l_hip_motor_respondable.stl"
    );
    meshFilesMap.set(
      "l_hip_motor_visual.STL",
      "/assets/robot2/meshes/l_hip_motor_visual.STL"
    );
    meshFilesMap.set(
      "l_hip_respondable.stl",
      "/assets/robot2/meshes/l_hip_respondable.stl"
    );
    meshFilesMap.set(
      "l_hip_visual.STL",
      "/assets/robot2/meshes/l_hip_visual.STL"
    );
    meshFilesMap.set(
      "l_shin_respondable.stl",
      "/assets/robot2/meshes/l_shin_respondable.stl"
    );
    meshFilesMap.set(
      "l_shin_visual.STL",
      "/assets/robot2/meshes/l_shin_visual.STL"
    );
    meshFilesMap.set(
      "l_shoulder_motor_respondable.stl",
      "/assets/robot2/meshes/l_shoulder_motor_respondable.stl"
    );
    meshFilesMap.set(
      "l_shoulder_motor_visual.STL",
      "/assets/robot2/meshes/l_shoulder_motor_visual.STL"
    );
    meshFilesMap.set(
      "l_shoulder_respondable.stl",
      "/assets/robot2/meshes/l_shoulder_respondable.stl"
    );
    meshFilesMap.set(
      "l_shoulder_visual.STL",
      "/assets/robot2/meshes/l_shoulder_visual.STL"
    );
    meshFilesMap.set(
      "l_thigh_respondable.STL",
      "/assets/robot2/meshes/l_thigh_respondable.STL"
    );
    meshFilesMap.set(
      "l_thigh_visual.STL",
      "/assets/robot2/meshes/l_thigh_visual.STL"
    );
    meshFilesMap.set(
      "l_upper_arm_respondable.STL",
      "/assets/robot2/meshes/l_upper_arm_respondable.STL"
    );
    meshFilesMap.set(
      "l_upper_arm_visual.STL",
      "/assets/robot2/meshes/l_upper_arm_visual.STL"
    );
    meshFilesMap.set(
      "neck_respondable.STL",
      "/assets/robot2/meshes/neck_respondable.STL"
    );
    meshFilesMap.set(
      "neck_visual.STL",
      "/assets/robot2/meshes/neck_visual.STL"
    );
    meshFilesMap.set(
      "pelvis_respondable.STL",
      "/assets/robot2/meshes/pelvis_respondable.STL"
    );
    meshFilesMap.set(
      "pelvis_visual.STL",
      "/assets/robot2/meshes/pelvis_visual.STL"
    );
    meshFilesMap.set(
      "r_foot_respondable.STL",
      "/assets/robot2/meshes/r_foot_respondable.STL"
    );
    meshFilesMap.set(
      "r_foot_visual.STL",
      "/assets/robot2/meshes/r_foot_visual.STL"
    );
    meshFilesMap.set(
      "r_forearm_respondable.STL",
      "/assets/robot2/meshes/r_forearm_respondable.STL"
    );
    meshFilesMap.set(
      "r_forearm_visual.STL",
      "/assets/robot2/meshes/r_forearm_visual.STL"
    );
    meshFilesMap.set(
      "r_hip_motor_respondable.STL",
      "/assets/robot2/meshes/r_hip_motor_respondable.STL"
    );
    meshFilesMap.set(
      "r_hip_motor_visual.STL",
      "/assets/robot2/meshes/r_hip_motor_visual.STL"
    );
    meshFilesMap.set(
      "r_hip_respondable.STL",
      "/assets/robot2/meshes/r_hip_respondable.STL"
    );
    meshFilesMap.set(
      "r_hip_visual.STL",
      "/assets/robot2/meshes/r_hip_visual.STL"
    );
    meshFilesMap.set(
      "r_shin_respondable.STL",
      "/assets/robot2/meshes/r_shin_respondable.STL"
    );
    meshFilesMap.set(
      "r_shin_visual.STL",
      "/assets/robot2/meshes/r_shin_visual.STL"
    );
    meshFilesMap.set(
      "r_shoulder_motor_respondable.STL",
      "/assets/robot2/meshes/r_shoulder_motor_respondable.STL"
    );
    meshFilesMap.set(
      "r_shoulder_motor_visual.STL",
      "/assets/robot2/meshes/r_shoulder_motor_visual.STL"
    );
    meshFilesMap.set(
      "r_shoulder_respondable.STL",
      "/assets/robot2/meshes/r_shoulder_respondable.STL"
    );
    meshFilesMap.set(
      "r_shoulder_visual.STL",
      "/assets/robot2/meshes/r_shoulder_visual.STL"
    );
    meshFilesMap.set(
      "r_thigh_respondable.STL",
      "/assets/robot2/meshes/r_thigh_respondable.STL"
    );
    meshFilesMap.set(
      "r_thigh_visual.STL",
      "/assets/robot2/meshes/r_thigh_visual.STL"
    );
    meshFilesMap.set(
      "r_upper_arm_respondable.STL",
      "/assets/robot2/meshes/r_upper_arm_respondable.STL"
    );
    meshFilesMap.set(
      "r_upper_arm_visual.STL",
      "/assets/robot2/meshes/r_upper_arm_visual.STL"
    );
    meshFilesMap.set(
      "spine_respondable.STL",
      "/assets/robot2/meshes/spine_respondable.STL"
    );
    meshFilesMap.set(
      "spine_visual.STL",
      "/assets/robot2/meshes/spine_visual.STL"
    );
    ...
    this.updateStatus(
      `Prepared ${
        this.meshFiles.size / 2
      } mesh file entries for automatic loading.`,
      "info"
    );
    ...
    try {
      ...
      this.updateStatus(
        "Error automatically loading Poppy Humanoid: " + error.message,
        "error"
      );
      const loadButtonText = document.getElementById("loadButtonText");
      if (loadButtonText) loadButtonText.textContent = "Load Robot";
  ...
  async parseURDF(urdfContent) {
    ...
    const childLinkNames = new Set(
      Object.values(this.joints).map((j) => j.child)
    );
    ...
    if (rootLinkName && this.links[rootLinkName]) {
      ...
      console.warn(
        "Could not determine a clear root link. Adding first link to scene root."
      );
  ...
  updateStatsDisplay() {
    document.getElementById("linkCount").textContent = Object.keys(
      this.links
    ).length;
    document.getElementById("jointCount").textContent = Object.keys(
      this.joints
    ).length;
  ...
  async createVisualMesh(visual) {
    ...
    let material = this.parseMaterial(
      visual.getElementsByTagName("material")[0]
    );
  ...
  async loadMeshGeometry(meshElement) {
    ...
    if (!meshFile) {
      console.warn(
        `Mesh file not found in provided files/map: ${meshPath}. Using fallback cube.`
      );
    ...
    try {
      ...
      const geometry = await this.loadMeshFile(
        meshFile.url || meshFile,
        meshExtension
      );
      ...
      this.updateStatus(
        `Error loading mesh ${meshBaseName}. Check console.`,
        "error"
      );
      return new THREE.BoxGeometry(0.1, 0.1, 0.1);
    }
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (typeof fileOrUrl === "string") {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Failed to fetch mesh from URL: ${fileOrUrl} - ${response.statusText}`
            );
          }
          if (extension === "stl") {
        ...
        // It's a File object (from user input)
        const reader = new FileReader();
        reader.onerror = reject;

        reader.onload = (e) => {
          data = e.target.result;
          this.processLoadedMeshData(
            data,
            extension,
            fileOrUrl.name,
            resolve,
  ...
  processLoadedMeshData(data, extension, filename, resolve, reject) {
    try {
      ...
      if (extension === "stl") {
        ...
        throw new Error(
          `Unsupported mesh format: .${extension} for file ${filename}`
        );
      }
  ...
  parseJoint(jointElement) {
    ...
    const parent = jointElement
      .getElementsByTagName("parent")[0]
      ?.getAttribute("link");
    const child = jointElement
    ...
    if (originElement) {
      if (originElement.hasAttribute("xyz")) {
        joint.origin.xyz = originElement
          .getAttribute("xyz")
          .split(" ")
          .map(Number);
      }
      if (originElement.hasAttribute("rpy")) {
        joint.origin.rpy = originElement
          .getAttribute("rpy")
          .split(" ")
          .map(Number);
      }
  ...
  applyOrigin(object, originData) {
    ...
    if (originData instanceof Element) {
      ...
      Array.isArray(originData.xyz) &&
      Array.isArray(originData.rpy)
    ) {
      xyz = originData.xyz;
      rpy = originData.rpy;
    } else {
  ...
  updateJoint(jointName, value) {
    ...
    if (joint.type === "revolute" || joint.type === "continuous") {
      ...
      childLink.quaternion.multiplyQuaternions(
        joint.initialRotation,
        quaternion
      );
...
window.showPopup = (message) => {
  // This global function now acts as a wrapper that ensures the viewer is ready.
  if (window.viewer && typeof window.viewer.showInfoPopup === "function") {
    window.viewer.showInfoPopup(message);
  } else {
    // Fallback for cases where viewer might not be ready (less likely with recommended placement)
    console.warn(
      "ProfessionalURDFViewer instance or showInfoPopup method not yet available. Displaying message via alert as fallback."
    );
    alert(message);
  }
};
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // 1. Initialize the viewer (this sets window.viewer)
  window.viewer = new ProfessionalURDFViewer();
  window.viewer.loadPoppyHumanoid(); // Or any other initial load you have

  // 2. NOW, attach the click listeners to the info icons.
  // This ensures that window.viewer is already set and ready for use.
  // Replace .fa-info with .info-icon (or .fa-info-circle if you prefer)
  document.body.addEventListener("click", (e) => {
    const icon = e.target.closest(".info-icon");
    if (icon) {
      const msg = icon.getAttribute("data-info");
      if (window.viewer && typeof window.viewer.showInfoPopup === "function") {
        window.viewer.showInfoPopup(msg);
      } else {
        alert(msg);
      }
    }
