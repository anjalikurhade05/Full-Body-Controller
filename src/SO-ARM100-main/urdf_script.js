    ...
    init() {
     ...
     connectWebSocket() {
        ...
        this.websocket.onopen = () => {
            console.log('WebSocket connection to PyBullet established.');
            this.websocketConnected = true;
            this.updateStatus('Connected to PyBullet simulation.', 'success');
        };

        this.websocket.onmessage = (event) => {
            console.log('Message from PyBullet:', event.data);
            // Handle any messages you might receive from the Python server
        };

        this.websocket.onclose = (event) => {
            console.log('WebSocket connection to PyBullet closed:', event.code, event.reason);
            this.websocketConnected = false;
            this.updateStatus('Disconnected from PyBullet. Trying to reconnect...', 'error');
...
async loadSOARM100() {
    this.updateStatus('Automatically loading SO-ARM100...', 'loading');

    const urdfPath = '/assets/SO-ARM100-main/urdf/so101_new_calib.urdf';

    const meshFilesMap = new Map([
        ['base_motor_holder_so101_v1.stl', '/assets/SO-ARM100-main/meshes/base_motor_holder_so101_v1.stl'],
        ['base_so101_v2.stl', '/assets/SO-ARM100-main/meshes/base_so101_v2.stl'],
        ['motor_holder_so101_base_v1.stl', '/assets/SO-ARM100-main/meshes/motor_holder_so101_base_v1.stl'],
        ['motor_holder_so101_wrist_v1.stl', '/assets/SO-ARM100-main/meshes/motor_holder_so101_wrist_v1.stl'],
        ['moving_jaw_so101_v1.stl', '/assets/SO-ARM100-main/meshes/moving_jaw_so101_v1.stl'],
        ['rotation_pitch_so101_v1.stl', '/assets/SO-ARM100-main/meshes/rotation_pitch_so101_v1.stl'],
        ['sts3215_03a_no_horn_v1.stl', '/assets/SO-ARM100-main/meshes/sts3215_03a_no_horn_v1.stl'],
        ['sts3215_03a_v1.stl', '/assets/SO-ARM100-main/meshes/sts3215_03a_v1.stl'],
        ['under_arm_so101_v1.stl', '/assets/SO-ARM100-main/meshes/under_arm_so101_v1.stl'],
        ['upper_arm_so101_v1.stl', '/assets/SO-ARM100-main/meshes/upper_arm_so101_v1.stl'],
        ['waveshare_mounting_plate_so101_v2.stl', '/assets/SO-ARM100-main/meshes/waveshare_mounting_plate_so101_v2.stl'],
        ['wrist_roll_follower_so101_v1.stl', '/assets/SO-ARM100-main/meshes/wrist_roll_follower_so101_v1.stl'],
        ['wrist_roll_pitch_so101_v2.stl', '/assets/SO-ARM100-main/meshes/wrist_roll_pitch_so101_v2.stl']
    ]);
    ...
    for (const [filenameInUrdf, url] of meshFilesMap.entries()) {
        ...
        this.meshFiles.set(filenameInUrdf, dummyFile);
        this.meshFiles.set(filenameInUrdf.split('.')[0], dummyFile);
    }
    ...
    try {
        ...
        this.updateStatus('SO-ARM100 loaded successfully!', 'success');
        const loadButtonText = document.getElementById('loadButtonText');
        if (loadButtonText) loadButtonText.textContent = 'Load Robot';

        if (typeof this.setSOARM100InitialPose === 'function') {
            this.setSOARM100InitialPose(); // Optional: Define separately if needed
        }

    } catch (error) {
        console.error('Error automatically loading SO-ARM100:', error);
        this.updateStatus('Error automatically loading SO-ARM100: ' + error.message, 'error');
        const loadButtonText = document.getElementById('loadButtonText');
        if (loadButtonText) loadButtonText.textContent = 'Load Robot';
    }
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    window.viewer.loadSOARM100(); // Call the new loading function
});
